										...
										<nav className="grid items-start px-4 text-sm font-medium">
											...
											{chatroom && chatroom.length > 0 ? (
												chatroom.map((room, index) => (
													<div key={`${activeView === 'group' ? room.ccjId : room.userchatId}_${index}`} className="flex justify-center items-center h-full">
														<button onClick={() => {
															if (activeView === 'group') {
																handleCommChat(room);
															} else {
						...
						{activeView === 'group' && (
							<div>
								...
								{isListExpanded && (
									<div>
										{chatroom && chatroom.map((room, index) => (
											<div key={`${room.ccjId}_${index}`} className="flex justify-center items-center h-full z-1">
												<button onClick={() => { handleCommChat(room); setIsListExpanded(false); }}
	                             				...
	                             				${selectedChat === room.roomNumber ? 'bg-gray-200' : 'hover:bg-gray-100 dark:hover:bg-gray-800'}`}>
													<div className="w-12 h-12 rounded-full overflow-hidden shrink-0 z-1">{(
														<img src={room.imgpath || room.profile_img || "/placeholder.svg"}
															alt={room.name} className="w-full h-full object-cover z-1" />)}
													</div>
													<span className="text-sm font-medium text-gray-800 dark:text-white z-1">
														{truncateString(room.roomname, 10)}
													</span>
												</button>
											</div>
